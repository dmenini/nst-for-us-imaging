...
parser.add_argument('--content',  dest='content',     type=int,     default=1,     						help='Content image number')
parser.add_argument('--style', 	  dest='style',       type=int,     default=11,     					help='Style image number')
...
def neural_style_transfer(args):
	...
	content_path = args.data_dir + '/' + str(args.content) + '.png'
	style_path = args.data_dir + '/' + str(args.style) + '.png'
	...
	content_image = image_preprocessing(content_path, 'content', args.input_size, c=3)
	content_seg = image_preprocessing(content_path, 'segmentation', MAX_SIZE, c=3)		# Needed MAX_SIZE to preserve labels	
	style_target = image_preprocessing(content_path, 'style', args.input_size, c=3)

	style_image = image_preprocessing(style_path, 'style', args.input_size, c=3)
	...
	mask_dict = extract_masks(content_seg, [style_image.shape[1], style_image.shape[2]], c=1, visualize=False)
	...
	def train_step(image, masks, style_features, content_features, weights):
		with tf.GradientTape() as tape:
			...
			if args.loss == 0:
				...
				loss_style = style_loss(outputs, style_features, weights[0]) + 0.3 * style_augmented_loss(outputs, masks, style_features, weights[0])
	...
	mse_score = mse(best_image, style_target)
	psnr_score = tf.image.psnr(best_image, style_target, max_val=1.0).numpy()[0]
	ssim_score = tf.image.ssim(best_image, style_target, max_val=1.0).numpy()[0]
	print("FINAL SCORE (epoch {}):\tMSE = {:.6f} \tPSNR = {:.6f} \tSSIM = {:.6f}\n".format(best_epoch, mse_score, psnr_score, ssim_score))

	file_name = args.save_dir + '/' + str(args.name) + str(args.content) + '_' + str(best_epoch) + '.png'
